...
int mysqlentry()
{
	...
	{	
        MYSQL *mysql;
        char query[100]="INSERT INTO workersDB VALUES ('";
        int t;
...
//	gtk_entry_set_text(GTK_ENTRY(entry_phno),PHNO);
	...
	gtk_box_pack_start(GTK_BOX(hbox4),button1,FALSE,FALSE,0);
	gtk_widget_show(button1);

	quit_button = gtk_button_new_with_label("Quit");
	g_signal_connect(G_OBJECT(quit_button),"clicked",G_CALLBACK(exiting),window);
	gtk_box_pack_start(GTK_BOX(hbox4),quit_button,FALSE,FALSE,0);
