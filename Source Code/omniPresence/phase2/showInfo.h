...
void search_with_empID_SI(int a)
{
...
void window_2_SI(char str1[],char str2[],char str3[])
{
	...
	button=gtk_button_new_with_label("back");
        g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(back_SI),(gpointer)window);
...
static void search_SI(GtkWidget *widget,GtkWidget *window)
{
	...
	{        
		if(arr[0]==1)
			search_with_empID_SI(1);
			
                if(arr[1]==1)
                        search_with_empID_SI(2);
			
		if(arr[2]==1)
               		search_with_empID_SI(3); 
...
void window_1_SI()
{
	...
	window=gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_title(GTK_WINDOW(window), "test1");
	gtk_container_set_border_width(GTK_CONTAINER(window),10);
	g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(gtk_main_quit), NULL);
	gtk_window_set_position (GTK_WINDOW (window), GTK_WIN_POS_CENTER);
	...
	label1=gtk_label_new("enter employee's id ");
	...
	g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(search_SI),(gpointer)window);
