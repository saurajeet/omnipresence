...
int attendanceSheetCreater(char month[4],char year[4])
{
	...
	if(!mysql_real_connect(sheetconn,"localhost","root","omni","omniPresence",0,NULL,0))
	{		printf("\nSHEETCRT:: Error Connecting Database... %s\n",mysql_error(sheetconn));
			exit(1);
	}
	t=mysql_query(sheetconn,query);
	if(t)
		printf("\nSHEETCRT:: Error in Query...%s\n",mysql_error(sheetconn));
	else
		printf("\nSHEETCRT:: Query Completed... Table created..\n");
