...
static void exiting_login(GtkWidget *widget,GtkWidget *window)
{
	login_retcode = 1;
	gtk_widget_hide(window);
	gtk_main_quit();
...
void login_window_content()
{
	...
	g_signal_connect(G_OBJECT(quit_button),"clicked",G_CALLBACK(exiting_login),window);
