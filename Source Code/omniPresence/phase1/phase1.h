...
pthread_mutex_t serialize_RP = PTHREAD_MUTEX_INITIALIZER;
int shutdown_realtimer = 0;
int shutdown_phase1 = 0;
struct info_MAC
{	char MAC[19];
	char date[100];
...
//	char  password[5] = "omni";		// MYSQL PASSWORD
	...
	while(difftime(duration,now)<60)               // SCAN FOR 1 MINUTE
		...
		{ 	
			...
			if (mysql_query(conn, steal))
                        {       fprintf(stderr, "BTSER:: Error in steal: %s. %s\n", steal,mysql_error(conn));
				info = 2;
                                goto shutdown_RFID;
                        }
			...
			strcat (query, "')");
			printf ("BTSER:: Query : %s.\n", query);
			if (mysql_query(conn, query))
			{	fprintf(stderr, "BTSER:: Error in Query: %s, %s\n",query,  mysql_error(conn));
				info = 2;
				goto shutdown_RFID;
			}
	...
	shutdown_RFID:
	pthread_mutex_lock (&serialize_RP);
	shutdown_realtimer = 1;
	pthread_mutex_unlock (&serialize_RP);
